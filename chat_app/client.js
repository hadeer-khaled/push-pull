...
apptitle.innerHTML = `Welcome ${username} to our Chat App `
...
mywebsocket.onmessage = function (event) {
    ...
    if (message_content.logout){
        content_div.innerHTML += `<div><h3 style="color: red ; padding:10px" class="rounded">${message_content.content}</h3>  </div>`
    }else if (message_content.sender == username) {
        content_div.innerHTML += `<div style="direction: rtl;"><h4 class="d-inline-block w-auto rounded" style=" background: antiquewhite;padding:10px ;">${message_content.content}</h4> </div>`
    }else{
